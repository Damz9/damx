...
os.system("clear")
autoketik(f"{biru}[{kuning}Warning{biru}] {G}Login Whatsapp Aplication")
time.sleep(3)
os.system("clear")
autoketik(f"{biru}[{kuning}Warning{biru}] {W}Gunakan Tools ini dengan baik")
time.sleep(3)
os.system("clear")
autoketik(f"""
{hijau}╔═╗{merah}┌─┐┌─┐┌┬┐  {biru}╦ ╦┬ ┬┌─┐┌┬┐{putih}┌─┐{kuning}┌─┐┌─┐ ┌─┐
{hijau}╚═╗{merah}├─┘├─┤│││  {biru}║║║├─┤├─┤ │ {putih}└─┐{kuning}├─┤├─┘ ├─┘
{hijau}╚═╝{merah}┴  ┴ ┴┴ ┴  {biru}╚╩╝┴ ┴┴ ┴ ┴ {putih}└─┘{kuning}┴ ┴┴   ┴ {hijau}V3.9
{abu}-----------------------------------------
{putih}[{biru}•{putih}] {biru}Author {putih}   : Idham Ramadhan
{putih}[{biru}•{putih}] {abu}Nick   {putih}   : Play
{putih}[{biru}•{putih}] {merah}You{putih}Tube {putih}  : Digimon World
{putih}[{biru}•{putih}] {ungu}Instagram {putih}: Pokemon Blue
...
for i in range(jum):
    ...
    print (f"{W}[{Y}✓{G}] Terkirim Spam Whatsapp")
